...
class grocery_Layout extends grocery_Model_Driver
{
	...
	protected $default_true_false_text 	= array('لا' , 'نعم');
...
//@todo have to do the Select {display_as} as a lang string		
		$input = "<select name='{$field_info->name}' class='chosen-select' data-placeholder='اختر إحدى القيم'>";
		...
		foreach ($rules As $rule)
		{
			...
			if ($result === FALSE)
			{
				if ( ! isset($this->_error_messages[$rule]))
				{
					...
					{
						$line = 'غير قادر على تحديد الخطأ الملائم لهذا الحقل.';
					}
				}
				else
				{
